...
-- Completed Orders
IF NOT EXISTS (SELECT 1 FROM orders WHERE total_amount = 10.25 AND note = 'Yesterday''s order')
BEGIN
    DECLARE @Order1 INT;
...
IF NOT EXISTS (SELECT 1 FROM orders WHERE total_amount = 6.50 AND note = 'Morning rush')
BEGIN
    DECLARE @Order2 INT;
    INSERT INTO orders (user_id, order_date, status, total_amount, note, version, created_at, updated_at)
...
-- More pending orders
IF NOT EXISTS (SELECT 1 FROM orders WHERE status = 'PENDING' AND note = 'Iced Mocha')
BEGIN
    INSERT INTO orders (user_id, order_date, status, total_amount, note, version, created_at, updated_at)
